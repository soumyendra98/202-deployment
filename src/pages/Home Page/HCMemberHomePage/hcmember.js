...
const HCMember = () => {
  ...
  useEffect(() => {
    axios
      .get("http://newBackendLB-982605735.us-east-1.elb.amazonaws.com:3010/users")
      .then((response) => {
  ...
  return (
    <div>
      <button style={{ float: "right" }} className="btn btn-danger" onClick={handleLogout}>
        Logout
      </button>
      <div className="page-container">
        <div className="member-list-container">
          <div className="nm-member-list">
            <h3>
              <br></br>
              <b>CHECK-IN/CHECK-OUT Members</b>
            </h3>
            <div className="search-bar">
              <input
                type="text"
                placeholder="Search for members to check the time"
              ...
              <div key={member._id} className="member-card">
                <div className="member-info">
                  <div className="member-name">
                    ...
                    <button className="member-btn">{member.type} </button>
                    <div className="member-actions">
                      {!status.get(member.emailAddress) && (
                        <button className="checkin-btn" onClick={() => handleCheckin(member)}>
                          Check In
                        </button>
                      )}
                      {status.get(member.emailAddress) && (
                        <button className="checkout-btn" onClick={() => handleCheckout(member)}>
                          Check Out
                        </button>
                      )}
                    </div>
        ...
        <div className="enrollment-container">
          <div class="nm-member-list nm-type">
            <h3>
              <b>Enroll Members/Non members for different Services or Free Trials</b>
            </h3>
            <div className="search-bar">
              <input
                type="text"
                placeholder="Search for members/non members "
              ...
              <div key={nonmember._id} class="member-card nm-type">
                <div class="nm-member-info">
                  <div class="nm-member-name">
                    ...
                    <Link
                      to={`/enrollmember?email=${nonmember.emailAddress}&type=${nonmember.type}`}
                    >
